...
function GroupChat({ groupID }) {
    ...
    const getMembers = () => {
        ...
        axios.get(`http://localhost:7979/groupChat/members/${groupID}`, {
            headers: {
    ...
    function getSelectedGroupChat(groupID) {
        axios.get(`http://localhost:7979/groupChat/getGroup/${groupID}`, {
            headers: {
                "x-access-token": localStorage.getItem('token'),
            },
        }).then((res) => {
            if (res.status === 200) {
                ...
                console.log('An error occurred while getting group chat:', res.data.message);
            }
        }).catch((err) => {
    ...
    function getMessages(groupID) {
        axios.get(`http://localhost:7979/groupMessages/getByGroup/${groupID}`, {
            headers: {
                "x-access-token": localStorage.getItem('token'),
            },
        }).then((res) => {
